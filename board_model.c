...
enum ResCodes initializeLevel(struct board* aboard) {
    ...
    for(int i = 0; i < 9; i++){
        int j = rand() % 3;
        // aboard->last_row+1 = Barrier
        placeItem(aboard, rand() % (aboard->last_row-3)+2, rand() % (aboard->last_col-3)+2,BC_FOOD_1+j,(j==2)?SYMBOL_FOOD_3:((j==1)?SYMBOL_FOOD_2:SYMBOL_FOOD_1),COLP_FOOD_1+j);
    }
